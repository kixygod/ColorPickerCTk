...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.navigation_frame_label = customtkinter.CTkLabel(
            self.navigation_frame, text="Color Picker", compound="left", font=customtkinter.CTkFont(size=15, weight="bold"))
        self.navigation_frame_label.grid(row=0, column=0, padx=20, pady=20)

        self.home_button = customtkinter.CTkButton(self.navigation_frame, corner_radius=0, height=40, border_spacing=10, text="Pick the color", fg_color="transparent", text_color=(
            "gray10", "gray90"), hover_color=("gray70", "gray30"), anchor="w", command=self.home_button_event)
        self.home_button.grid(row=1, column=0, sticky="ew")

        self.frame_2_button = customtkinter.CTkButton(self.navigation_frame, corner_radius=0, height=40, border_spacing=10, text="About", fg_color="transparent", text_color=(
            "gray10", "gray90"), hover_color=("gray70", "gray30"), anchor="w", command=self.frame_2_button_event)
        self.frame_2_button.grid(row=2, column=0, sticky="ew")

        self.appearance_mode_menu = customtkinter.CTkOptionMenu(self.navigation_frame, values=[
        ...
        self.right_panel = customtkinter.CTkFrame(self.home_frame)
        ...
        # self.colors = [("#0000FF"),("#0000FF"),("#0000FF"),("#0000FF"),("#0000FF"),("#0000FF"),("#0000FF"),("#0000FF"),("#0000FF"),("#0000FF"),("#0000FF")]
        self.colors = []
        self.scrollable_frame_boxes = []

        self.photo_frame = customtkinter.CTkFrame(self.home_frame)
        self.photo_frame.grid(row=0, column=0, padx=(
    ...
    def load_image(self):
        self.file_path = filedialog.askopenfilename(filetypes=[("Изображения", "*.png;*.jpg;*.jpeg")])
        if self.file_path:
            ...
            self.parent_frame_aspect_ratio = self.parent_frame_width / self.parent_frame_height
            ...
            if self.image.width / self.image.height >= self.parent_frame_aspect_ratio:
                ...
                self.desired_height = int(self.parent_frame_width / self.image.width * self.image.height)
            else:
                self.desired_height = self.parent_frame_height
                self.desired_width = int(self.parent_frame_height / self.image.height * self.image.width)

            self.photo = customtkinter.CTkImage(light_image=self.image, dark_image=self.image, size=(self.desired_width, self.desired_height))
            self.image_label.configure(image=self.photo)
            self.resized_image = self.image.resize((self.desired_width, self.desired_height), Image.BICUBIC)
    ...
    def get_color(self, event):
        if self.file_path:
            ...
            y = event.y
            # Debug
            # print(f"x {x} | rootx {self.image_label.winfo_x()} | x {event.x}")
            # print(f"y {y} | rooty {self.image_label.winfo_y()} | y {event.y}")
    ...
    def create_line(self, color_code):
        ...
        color_label.grid(row=self.index_line, column=0, padx=(
            10, 10), pady=(10, 10), sticky="w")
        code_label = customtkinter.CTkLabel(
            self.scrollable_frame, text=color_code)
        code_label.grid(row=self.index_line, column=1, padx=(
